  ...
  def main(args: Array[String]): Unit = {
    ...
    val boolToAlertDStream: DStream[(Boolean, CouponAlertInfo)] = midToIterEventLogDStream.mapPartitions(partition => {
      partition.map { case (mid, iter) =>
        ...
        //生成疑似预警日志
        (uids.size() >= 2 && bool, CouponAlertInfo(mid, uids, itemIds, events, System.currentTimeMillis()))
      }
    })
    boolToAlertDStream

    //8.筛选符合预警需求的数据
    val couponAlterInfoDStream: DStream[CouponAlertInfo] = boolToAlertDStream.filter(_._1).map(_._2)
